...
void free_memory(long double **ZON, long double **XDOM, long double **YDOM,
                 int **ZMAP, long double **QMAP, long double **MIU,
                 long double **THETA, long double **FI, long double **W,
                 long double **LIST, long double **MFLUX, long double **MFLOW,
                 long double **XFLOW, long double **YFLOW);

//////////////////////////////////////////////////////// LOAD PROBLEM FROM FILE
int input_by_txt(int *N, int *nz, long double **ZON, int *nxr,
                 long double **XDOM, int *nyr, long double **YDOM, int **ZMAP,
                 long double **QMAP, long double *BC, long double *tol,
                 const char *filename);

/////////////////////////////////////////////////////////// GENERATE QUADRATURE
int quad(int N, long double **MIU, long double **THETA, long double **CHI,
         long double **W, long double **LIST);

/////////////////////////////////////////////////////////// DD ITERATIVE SCHEME
int dd(int N, int nz, long double ZON[], int nxr, long double XDOM[], int nyr,
       long double YDOM[], int ZMAP[], long double QMAP[], long double BC[],
       long double tol, long double MIU[], long double THETA[], long double W[],
       long double **MFLUX, long double **MFLOW, long double **XFLOW,
       long double **YFLOW, int *ITER, long double *cpu_time);

////////////////////////////////////////////////////////// AUXILIARY FUNCTIONS
void json_output(int N, int nxr, long double XDOM[], int nyr,
                 long double YDOM[], int status, int ITER, long double cpu_time,
                 long double MFLUX[], long double MFLOW[], long double XFLOW[],
                 long double YFLOW[]);
...
int main(int argc, char *argv[]) {
  ...
  if (argc != 2) {
    // Usage: ./RM_CN <input>
    status = 1;
    json_output(N, nxr, XDOM, nyr, YDOM, status, ITER, cpu_time, MFLUX, MFLOW,
                XFLOW, YFLOW);
    return 0;
  }
  ...
  // LOAD PROBLEM
  filename = argv[1];
  status = input_by_txt(&N, &nz, &ZON, &nxr, &XDOM, &nyr, &YDOM, &ZMAP, &QMAP,
                        BC, &tol, filename);
  if (status != 0) {
    json_output(N, nxr, XDOM, nyr, YDOM, status, ITER, cpu_time, MFLUX, MFLOW,
                XFLOW, YFLOW);
  ...
  if (status != 0) {
    json_output(N, nxr, XDOM, nyr, YDOM, status, ITER, cpu_time, MFLUX, MFLOW,
                XFLOW, YFLOW);
    free_memory(&ZON, &XDOM, &YDOM, &ZMAP, &QMAP, &MIU, &THETA, &FI, &W, &LIST,
                &MFLUX, &MFLOW, &XFLOW, &YFLOW);
    return 0;
  };

  // ITERATIVE SCHEME
  status = dd(N, nz, ZON, nxr, XDOM, nyr, YDOM, ZMAP, QMAP, BC, tol, MIU, THETA,
              W, &MFLUX, &MFLOW, &XFLOW, &YFLOW, &ITER, &cpu_time);
  if (status != 0) {
    json_output(N, nxr, XDOM, nyr, YDOM, status, ITER, cpu_time, MFLUX, MFLOW,
                XFLOW, YFLOW);
    free_memory(&ZON, &XDOM, &YDOM, &ZMAP, &QMAP, &MIU, &THETA, &FI, &W, &LIST,
...
int input_by_txt(int *N,              // Quadrature order
  ...
  // ZONE FILLING
  *ZON = malloc(sizeof(long double) * (*nz * 2));
  if (*ZON == NULL)
    return 3;
  double st, ss;
  int zone_count = 0;
  for (int z = 0; z < *nz; z++) {
    ...
    (*ZON)[z * 2] = (long double)st;
    (*ZON)[z * 2 + 1] = (long double)ss;
  ...
  *XDOM = malloc(sizeof(long double) * (*nxr * 2));
  ...
  for (int xr = 0; xr < *nxr; xr++) {
    ...
    (*XDOM)[xr * 2] = (long double)len;
    (*XDOM)[xr * 2 + 1] = (long double)nodes;
    rx_count = rx_count + 1;
  }
  if (rx_count != *nxr) {
    free(*ZON);
  ...
  *YDOM = malloc(sizeof(long double) * (*nyr * 2));
  ...
  for (int yr = 0; yr < *nyr; yr++) {
    ...
    (*YDOM)[yr * 2] = (long double)len;
    (*YDOM)[yr * 2 + 1] = (long double)nodes;
    ry_count = ry_count + 1;
  }
  if (ry_count != *nyr) {
    free(*ZON);
    ...
    // Must be provided the same number of rows as the number of regions in
    // Y.");
    return 2;
  }
...
int quad(int N,               // Quadrature order
  ...
  *LIST = malloc(sizeof(long double) * N / 2);
  if (*MIU == NULL || *THETA == NULL || *FI == NULL || *W == NULL ||
      *LIST == NULL)
    return 3;
...
int dd(int N,                // Quadrature order
  ...
  *YFLOW = malloc(sizeof(long double) * ((ntc_y + 1) * ntc_x) * M);
  if (*MFLUX == NULL || *MFLOW == NULL || *XFLOW == NULL || *YFLOW == NULL ||
      S == NULL) {
    if (S != NULL)
      free(S);
    return 3;
    ...
    *ITER = *ITER + 1;
    ...
    for (int m = 0; m < M; m++) {
      ...
      // 2. SE -> NW SWEEP
        ...
        int j_b = 0, j_f = j_b + 1, i_b = ntc_x - 1, i_f = i_b + 1, nc_y, nc_x,
            z;
        long double h_y, h_x, miu, theta, alfa_x, alfa_y, st, q, len_y, len_x;
        long double mflow, xflow, yflow;
        miu = MIU[m];
      ...
      // 4. NW -> SE SWEEP
        ...
        int j_b = ntc_y - 1, j_f = j_b + 1, i_b = 0, i_f = i_b + 1, nc_y, nc_x,
            z;
        long double h_y, h_x, miu, theta, alfa_x, alfa_y, st, q, len_y, len_x;
        long double mflow, xflow, yflow;
        miu = MIU[m];
    ...
    for (int ry = 0; ry < nyr; ry++) {
      ...
      for (int j = 0; j < nc_y; j++) {
        for (int m = 0; m < M / 4; m++) {
          if (BC[0] == -1.0) {
            (*XFLOW)[M * ((ntc_x + 1) * j_b + 0) + m] =
                (*XFLOW)[M * ((ntc_x + 1) * j_b + 0) + M / 4 + m];
            (*XFLOW)[M * ((ntc_x + 1) * j_b + 0) + 3 * M / 4 + m] =
                (*XFLOW)[M * ((ntc_x + 1) * j_b + 0) + M / 2 + m];
          }
          if (BC[2] == -1.0) {
            (*XFLOW)[M * ((ntc_x + 1) * j_b + ntc_x) + M / 4 + m] =
                (*XFLOW)[M * ((ntc_x + 1) * j_b + ntc_x) + m];
            (*XFLOW)[M * ((ntc_x + 1) * j_b + ntc_x) + M / 2 + m] =
                (*XFLOW)[M * ((ntc_x + 1) * j_b + ntc_x) + 3 * M / 4 + m];
          }
        }
        j_b = j_b + 1;
      }
    ...
    for (int rx = 0; rx < nxr; rx++) {
      ...
      for (int i = 0; i < nc_x; i++) {
        for (int m = 0; m < M / 4; m++) {
          if (BC[1] == -1.0) {
            (*YFLOW)[M * (ntc_x * 0 + i_b) + m] =
                (*YFLOW)[M * (ntc_x * 0 + i_b) + 3 * M / 4 + m];
            (*YFLOW)[M * (ntc_x * 0 + i_b) + M / 4 + m] =
                (*YFLOW)[M * (ntc_x * 0 + i_b) + M / 2 + m];
          }
          if (BC[3] == -1.0) {
            (*YFLOW)[M * (ntc_x * ntc_y + i_b) + 3 * M / 4 + m] =
                (*YFLOW)[M * (ntc_x * ntc_y + i_b) + m];
            (*YFLOW)[M * (ntc_x * ntc_y + i_b) + M / 2 + m] =
                (*YFLOW)[M * (ntc_x * ntc_y + i_b) + M / 4 + m];
          }
        }
        i_b = i_b + 1;
      }
      ...
      for (int j = 0; j < nc_y; j++) {
        ...
        for (int rx = 0; rx < nxr; rx++) {
          ...
          for (int i = 0; i < nc_x; i++) {
            ...
            if (fabs(1 - faux / mflux) > ERR)
              ERR = fabs(1 - faux / mflux);
          ...
          for (int rx = 0; rx < nxr; rx++) {
            ...
            for (int i = 0; i < nc_x; i++) {
              if (i_b == ntc_x - 1) {
                ...
                printf(" %.10Le,", MFLUX[ntc_x * j_b + i_b]);
              i_b = i_b + 1;
            }
          }
          j_b = j_b + 1;
            ...
            for (int rx = 0; rx < nxr; rx++) {
              ...
              for (int i = 0; i < nc_x; i++) {
                if (i_b == ntc_x - 1) {
                  ...
                  printf(" %.10Le,", MFLOW[M * (ntc_x * j_b + i_b) + m]);
                i_b = i_b + 1;
              }
            }
            j_b = j_b + 1;
            ...
            for (int rx = 0; rx < nxr; rx++) {
              ...
              for (int i = 0; i < nc_x; i++) {
                if (i_b == ntc_x - 1)
                  ...
                  printf(" %.10Le,", YFLOW[M * (ntc_x * j_b + i_b) + m]);
                i_b = i_b + 1;
              }
            }
            j_b = j_b + 1;
